...
export default function MakananPage() {
    ...
    const { inventory, updateInventoryItem, deleteInventoryItem } = useExpense();
    const [isInvModalOpen, setIsInvModalOpen] = useState(false);
    const [selectedItemToBuy, setSelectedItemToBuy] = useState<InventoryItem | null>(null);

    // If tab is expenses, just show the transparent CategoryLayout wrapper
    if (activeTab === 'expenses') {
        return (
            <>
                <div className="relative z-10 bg-slate-900/50 p-2 mx-6 mt-6 rounded-xl flex border border-white/5 backdrop-blur-sm">
                    <button
                        onClick={() => setActiveTab('expenses')}
                        className="flex-1 py-2 rounded-lg text-sm font-medium bg-cyan-500/20 text-cyan-400 shadow-inner"
                    >
                        Pengeluaran
                    </button>
                    <button
                        onClick={() => setActiveTab('inventory')}
                        className="flex-1 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white"
                    >
                        Belanjaan
                    </button>
                </div>
                <CategoryLayout
                    ...
                    category="makanan"
                    description="Bahan masak, snack, air galon."
                />
            </>
        );
    }
        ...
        <div className="min-h-full">
            <div className="sticky top-0 z-40 bg-slate-900/80 backdrop-blur-md -mx-6 px-6 py-4 border-b border-white/5 mb-6">
                <div className="flex justify-between items-end mb-4">
                    <div>
                        <h1 className="text-2xl font-bold text-white">Daftar Belanja</h1>
                        <p className="text-xs text-slate-400">Cek barang yang perlu dibeli</p>
                    </div>
                    <div className="text-right">
                        <div className="text-xs text-slate-400">Total Items</div>
                        <div className="text-lg font-bold text-emerald-400">{inventory.length}</div>
                    </div>
                </div>
                ...
                <div className="bg-slate-900/50 p-1 rounded-xl flex border border-white/10">
                    <button
                        onClick={() => setActiveTab('expenses')}
                        className="flex-1 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white transition-colors"
                    >
                        Pengeluaran
                    </button>
                    <button
                        onClick={() => setActiveTab('inventory')}
                        className="flex-1 py-2 rounded-lg text-sm font-medium bg-emerald-500/20 text-emerald-400 shadow-inner transition-colors"
                    >
                        Belanjaan
                    </button>
                </div>
            ...
            <button
                ...
                className="fixed bottom-24 right-6 w-14 h-14 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-full shadow-[0_0_20px_rgba(16,185,129,0.5)] flex items-center justify-center text-white text-2xl hover:scale-105 active:scale-95 transition-all z-40"
            >
                +
            </button>
